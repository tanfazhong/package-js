 ...
 *
 * build之前的JS必须做的工作
 * 1.先加载Package.js及所有Lib的_nsconf_.js文件，再按依赖关系排序Package
...
var Package={
    ...
    _libs={},
    /**
     * 所有已经加载的pkg,以Package Id为键，以PackageObject为值
     * 作为快速查找Package的cache
     */
    _pkgs={},
    /**
     *    所有的(包括正在加载的)PackageMeta对象
     * 以id为键
     */
    _pkgMetas={},
    /**
    ...
    _loaders={
      ...
      wui4ie6:{
        ...
        getAssetDeps:function () {
          return {
            _style:Package._IE6?'ie6.css':'style.css',
            tpl:'tpl.html'
          };
        }
      }
...
Package.loaders=_loaders;
Package.fetchers=_fetchers;
Package._nsbackup=_nsbackup;
Package._waits=_waits;
Package._libs=_libs;
Package._pkgs=_pkgs;
Package._pkgMetas=_pkgMetas;
Package._nsconfs=_nsconfs;
...
//占位
_libs.Package=Package;
_pkgs.Package=Package;
_pkgMetas.Package={
  pid:'Package',
  body:arguments.callee
};
_pkgs['Package._nsconf_']=Package._nsconf_=_nsconfs.Package={
  rootPath:(typeof __dirname!=='undefined' && !G.document)?__dirname+'/../':_getLibRootPath('/Package/init.js')
};


//将Package导出成全局变量,之后可以用Package.ns('Package',false)来还原旧值
ns('Package',Package);
...
/**
 * define a package
 * @param {String} pid Package Id,Package's full namespace.
 * @param {[]String} [deps] Package dependences,必须是数组类型
 * @param {Object} [assetDeps] Package asset dependences
 * @param {Function} [body] Package factory function
 */
function define(pid) {
  var argv=arguments,
      argc=argv.length,
  ...
  if (!pkgMeta.assets) {
    if (pkgMeta.assetDeps) {//nsconf里有预定义的assetDeps,所以在package加载前就有了assetDeps
      if (assetDeps) {
        extend(pkgMeta.assetDeps || {},assetDeps);
      }
      assetDeps=pkgMeta.assetDeps;//写在package中的assetDeps和useLoaders中loader的预先写到meta上的assetDeps
    }
...
function initit(pkgMeta) {
  ...
  var pkg=pkgMeta.body.apply(pkgMeta,depPkgs || []),
      pid=pkgMeta.pid;
  if (!pkg) {
    throw new Error("Invalid package return:"+pid);
  }
  pkg._pkgMeta_=pkgMeta;
  if (pid.indexOf('.')<0) {//it is Root Package
...
function nsConfig(rootNS,nsconf) {
  ...
  if (!nsconf.rootPath) {//打包之后会加上这个path,就不会去检查
    ...
    if (!nsconf.rootPath) {//仍然查找不到rootPath,报错吧
      throw 'Package root path can not be resolved:'+rootNS;
    }
  }
...
/**
 * 向处提供一个Pkg,用于运行时动态创建Package,
 * 动态创建的Package没有packageMeta,也没有nsconf及对根命名空间的依赖
 * 相当于ns方法，但ns方法注册的对象不能在imports及define的deps中使用
 * provide方法注册的Package可以被imports
 * @param {String} id Package id
 * @param {Object} pkg  Package object
 * @returns {Object} pkg
 */
function provide(id,pkg) {
  if (_pkgs.hasOwnProperty(id)) { //已经存在的Package
...
/**
 * 导入一系列Package后执行回调，并将pkg的引用按次序传递给回调函数作为参数
 * @param {[]String} pkgIds PackageId 数组
